simulate <- function(theta,init.state,times) {
  ...
  traj$prev <-(traj$Af +traj$Lf)/(traj$Sf + traj$Af +traj$Lf)
...
rPointObs <- function(model.point, theta){
  ...
  obs.point <- rbinom(1,round(model.point[["Sf"]]+model.point[["Af"]]+model.point[["Lf"]]),model.point[["prev"]])/round(model.point[["Sf"]]+model.point[["Af"]]+model.point[["Lf"]])
...
herpes_det_f <- fitmodel(name, state.names, theta.names,simulate, rPointObs, dprior,dPointObs)  
